...
``` sql
with p_users as (SELECT DISTINCT ua.time::date as date,
                                 count(distinct ua.user_id) filter (WHERE ua.action = 'create_order') as paying_users
                 FROM   user_actions ua
                 WHERE  order_id not in (SELECT order_id
                                         FROM   user_actions
                                         WHERE  action = 'cancel_order')
                 GROUP BY date) , orders_count_t as (SELECT date,
                                           count(user_id) filter (WHERE orders_count = 1) as single_order_users ,
                                           count(user_id) filter (WHERE orders_count > 1) as several_order_users
                                    FROM   (SELECT time::date as date,
                                            ...
                                            ORDER BY date) as orders_count_table
                                    GROUP BY date)
SELECT pu.date,
       round(100.0 * single_order_users / paying_users, 2) as single_order_users_share ,
       round(100.0 * several_order_users / paying_users, 2) as several_orders_users_share
FROM   p_users pu join orders_count_t oc using(date)
ORDER BY date
```
Вариант верного решения:
``` sql
...
``` sql
with total_day_orders_table as (
select time::date as date, count(order_id) as total_day_orders
from user_actions
WHERE  order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
GROUP by date
)
, first_orders_count_table as (
select first_order_date as date, count(user_id) as first_orders_count
from (select user_id, min(time)::date as first_order_date
from user_actions
WHERE  order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
group by user_id) t
group by first_order_date
)
, first_date_orders_count_table as (select date, sum(orders_count)::int as orders_count from (
select t2.user_id, first_action_date as date, coalesce(orders_count, 0) as orders_count
from
(select user_id, min(time)::date as first_action_date
from user_actions
group by user_id) t2
left join
(select user_id, time::date as date, count(order_id) as orders_count
from user_actions
WHERE  order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
group by user_id, date) t3 on t2.user_id = t3.user_id and t2.first_action_date = t3.date
) t4 group by date)

select tdo.date, tdo.total_day_orders as orders, foc.first_orders_count as first_orders, fdoc.orders_count as new_users_orders
, round(100.0 * foc.first_orders_count / tdo.total_day_orders, 2) as first_orders_share
, round(100.0 * fdoc.orders_count / tdo.total_day_orders, 2) as new_users_orders_share
from total_day_orders_table as tdo join first_orders_count_table as foc using(date) 
join first_date_orders_count_table as fdoc using(date)
order by date
```
Вариант верного решения:
``` sql
...
``` sql
with revenue_tax_table as (select creation_time::date as date, sum(order_price) as revenue, sum(order_tax) as tax
from orders join (SELECT o.order_id, sum(p.price) as order_price, sum(p.tax) as order_tax
FROM   (SELECT order_id, unnest(product_ids) as product_id FROM   orders) as o join (SELECT product_id, name, price
, case when name in ('сахар', 'сухарики', 'сушки', 'семечки', 'масло льняное', 'виноград', 'масло оливковое', 'арбуз', 'батон', 'йогурт', 'сливки', 'гречка', 'овсянка', 'макароны', 'баранина', 'апельсины', 'бублики', 'хлеб', 'горох', 'сметана', 'рыба копченая', 'мука', 'шпроты', 'сосиски', 'свинина', 'рис', 'масло кунжутное', 'сгущенка', 'ананас', 'говядина', 'соль', 'рыба вяленая', 'масло подсолнечное', 'яблоки', 'груши', 'лепешка', 'молоко', 'курица', 'лаваш', 'вафли', 'мандарины') then round(price / 1.1 * 0.1, 2)
else round(price / 1.2 * 0.2, 2) end as tax
...
, couriers_bonus_table as (select date, count(courier_id) filter (where courier_orders_count >= 5) as couriers_with_bonus
from (select time::date as date, courier_id, count(ORDER_id) as courier_orders_count
from courier_actions
...
, orders_count_table as (select time::date as date, count(order_id) filter (where action = 'deliver_order') as delivered_orders,
count(order_id) filter (where action = 'accept_order') as created_orders
from courier_actions
where order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
group by date)

, costs_table as (select date, case when date BETWEEN '2022-08-01' and '2022-08-31' then 120000 + sum(variable_costs) else 150000 + sum(variable_costs) end as costs
from (select date, case when date BETWEEN '2022-08-01' and '2022-08-31' 
then created_orders * 140 + delivered_orders * 150 + couriers_with_bonus * 400
else created_orders * 115 + delivered_orders * 150 + couriers_with_bonus * 500 end as variable_costs
from orders_count_table join couriers_bonus_table using(date)) t
group by date)
...
, sum(tax) over (order by date) as total_tax, sum(gross_profit) over (order by date) as total_gross_profit
, round(100.0 * gross_profit / revenue, 2) as gross_profit_ratio
, round(100.0 * sum(gross_profit) over (order by date) / sum(revenue) over (order by date), 2) as total_gross_profit_ratio
from(select date, revenue, costs, tax, revenue - costs - tax as gross_profit
from revenue_tax_table join costs_table using(date)) t
order by date
```
![alt text](<Daily Gross Profit and Gross Profit Ratio.png>)
![alt text](<Total Gross Profit and Total Gross Profit Ratio.png>)
## Маркетинговые метрики Задача 1.
Задание:

На основе таблицы user_actions рассчитайте метрику CAC для двух рекламных кампаний.
``` sql
with new_users_table as (select 'Кампания № 1' as ads_campaign, user_id, order_id, action
from user_actions
...
10105, 10122, 10134, 10135) and action = 'create_order' and order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
union
select 'Кампания № 2' as ads_campaign, user_id, order_id, action
from user_actions
...
from (select ads_campaign, user_id
from new_users_table
where action = 'create_order' and order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')) t
...
9935, 9939, 9958, 9959, 9961, 9983, 10027, 10033, 10038, 10045, 10047, 10048, 10058, 
10059, 10067, 10069, 10073, 10075, 10078, 10079, 10081, 10092, 10106, 10110, 10113, 10131))
, orders_price_table as (SELECT o.order_id,
       sum(p.price) as order_price
FROM   (SELECT order_id,
               unnest(product_ids) product_id
        FROM   orders) as o join products p
...
select ads_campaign, round((sum(order_price) - 250000) / 250000 * 100.0, 2) as roi
from new_users_table join orders_price_table using(order_id)
where action = 'create_order' and order_id not in (SELECT order_id FROM user_actions WHERE action = 'cancel_order')
group by ads_campaign
...
``` sql
with new_users_table as (select case when user_id in (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732, 8739, 8741, 
8750, 8751, 8752, 8770, 8774, 8788, 8791, 8804, 8810, 8815, 8828, 8830, 8845, 
8853, 8859, 8867, 8869, 8876, 8879, 8883, 8896, 8909, 8911, 8933, 8940, 8972, 
8976, 8988, 8990, 9002, 9004, 9009, 9019, 9020, 9035, 9036, 9061, 9069, 9071, 
9075, 9081, 9085, 9089, 9108, 9113, 9144, 9145, 9146, 9162, 9165, 9167, 9175, 
...
, user_id, order_id, action, time::date as date
from user_actions
where time::date BETWEEN '2022-09-01' and '2022-09-07' 
...
select ads_campaign, round(avg(avg_user_check), 2) as avg_check
from(select ads_campaign, user_id, avg(order_price) as avg_user_check
from new_users_table join orders_price_table using(order_id)
where date BETWEEN '2022-09-01' and '2022-09-07' and ads_campaign is not null
group by ads_campaign, user_id) as t
group by ads_campaign
order by avg_check desc
```
![alt text](<Average Check.png>)
## Задача 4.
Задание:
...
9935, 9939, 9958, 9959, 9961, 9983, 10027, 10033, 10038, 10045, 10047, 10048, 10058, 
10059, 10067, 10069, 10073, 10075, 10078, 10079, 10081, 10092, 10106, 10110, 10113, 10131) then  'Кампания № 2' end as ads_campaign
, user_id, order_id, action, time::date as dt, min(time::date) OVER (PARTITION BY user_id) as start_date
from user_actions)

SELECT ads_campaign, start_date, dt - start_date as day_number,
round(count(distinct user_id)::decimal / max(count(distinct user_id)) OVER (PARTITION BY start_date, ads_campaign), 2) as retention
    FROM   (SELECT user_id, ads_campaign, dt, start_date
        FROM   new_users_table
        where ads_campaign is not null) t
...
``` sql
-- SELECT ads_campaign, round(250000.0 / count(distinct user_id), 2) as cac, count(distinct user_id)
-- FROM   (SELECT user_id, order_id, action,
--               case when user_id in (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732,
--                                      8739, 8741, 8750, 8751, 8752, 8770, 8774, 8788, 8791,
--                                      8804, 8810, 8815, 8828, 8830, 8845, 8853, 8859, 8867,
--                                      8869, 8876, 8879, 8883, 8896, 8909, 8911, 8933, 8940,
...
--               count(action) filter (WHERE action = 'cancel_order') OVER (PARTITION BY order_id) as is_canceled
--         FROM   user_actions) t1
-- WHERE  ads_campaign is not null and is_canceled = 0
-- GROUP BY ads_campaign
-- ORDER BY cac desc
-- ads_campaign
-- cac
...
with temp_table as (SELECT user_id, order_id, action, time::date as dt, order_price,
               case when user_id in (8631, 8632, 8638, 8643, 8657, 8673, 8706, 8707, 8715, 8723, 8732,
                                     8739, 8741, 8750, 8751, 8752, 8770, 8774, 8788, 8791,
                                     8804, 8810, 8815, 8828, 8830, 8845, 8853, 8859, 8867,
                                     8869, 8876, 8879, 8883, 8896, 8909, 8911, 8933, 8940,
               ...
               count(action) filter (WHERE action = 'cancel_order') OVER (PARTITION BY order_id) as is_canceled
FROM   user_actions
join (SELECT o.order_id, sum(p.price) as order_price
FROM   (SELECT order_id,
               unnest(product_ids) product_id
        FROM   orders) as o join products p
        ON o.product_id = p.product_id
GROUP BY o.order_id) orders_price_table using(order_id))

select ads_campaign, dt, 
round(sum(revenue) over (PARTITION by ads_campaign order by dt) / max(users_count) over (PARTITION by ads_campaign), 2) as cumulative_arppu,
round(250000.0 / max(users_count) over (PARTITION by ads_campaign), 2) as cac
from (
select ads_campaign, dt, count(distinct user_id) as users_count, sum(order_price) as revenue
from temp_table
where ads_campaign is not null and is_canceled = 0
group by ads_campaign, dt
      ) t  
